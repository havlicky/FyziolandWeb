...
$query = "  SELECT
                ...
                s.name AS title,
                r.date,
                DATE_FORMAT(CAST(CONCAT(r.hour, ':', r.minute, ':00') AS TIME), '%k:%i') AS timeFrom,
                al.displayName,
                (IFNULL((SELECT 
                 ...
                 WHERE
                   	(SELECT internalNote FROM reservations WHERE reservations.id = :id) LIKE CONCAT('%',adminLogin.shortcut,'%') COLLATE utf8_czech_ci LIMIT 1        	 
                   
                ),'')) as puvodniTerapeut
            FROM reservations r
            LEFT JOIN services AS s ON s.id = r.service
            LEFT JOIN adminLogin AS al ON r.personnel = al.id
            WHERE
                r.id=:id2 AND
                r.active = 1";                
...
$stmt->bindParam(":id2", $_POST["id"], PDO::PARAM_STR);
$stmt->execute();
$result = $stmt->fetch(PDO::FETCH_OBJ);
...
$replacements = array(
    ...
    "#sluzba#" => $result->title,
    "#cas#" => $result->timeFrom,
    "#terapeut#" => $result->displayName,
    "#user#" => $resultAdminUser->displayName,
    "#puvodniterapeut#" => $result->puvodniTerapeut
);
