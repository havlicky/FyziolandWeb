...
if (empty($_GET["email"]) || empty($_GET["hsh"])) {
    ...
    $stmt->bindParam(":deleteHash", $_GET["hsh"], PDO::PARAM_STR);
    $stmt->execute();
    ...
    if ($reservationCancelDeadline < $now) {
        $messageBox->addText("Vaše rezervace zrušena nebyla, jelikož rezervaci lze zrušit maximálně 48 hodin předem. Kontaktujte nás prosím telefonicky.");
        $messageBox->setClass("alert-danger");
        ...
        $mail->Subject = "Zrušení rezervace - ". (new DateTime())->createFromFormat("Y-m-d", $result->date)->format("j. n. Y") . " " . $result->hour . ":" . str_pad($result->minute, 2, "0", STR_PAD_LEFT);
        ...
        $mail->Body .= "<tr><td style='background-color: #81ae37; color: white; border-top: 1px solid white; padding-left: 10px; padding-right: 10px; padding-top: 3px; padding-bottom: 3px;'>Rezervaci zrušil</td><td style='border: 1px solid #65921b; padding-left: 10px; padding-right: 10px; padding-top: 3px; padding-bottom: 3px;'>Klient odkazem z emailu</td></tr>";
        $mail->Body .= "</table>";
        $mail->Body .= "<br><br>";
        ...
        $messageBox->addText("Vaše rezervace byla úspěšně odstraněna.");
